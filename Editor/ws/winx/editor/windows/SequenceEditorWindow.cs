...
namespace ws.winx.editor.windows
{
	...
	public class SequenceEditorWindow : EditorWindow
	{
		...
		//Event
		private static SequenceEvent __eventDummy;
		...
		public static void ShowWindow ()
		{
			...
			if (Selection.activeGameObject != null) {
				__sequencer = Selection.activeGameObject.GetComponent<Sequencer> ();
				...
				if (__sequencer != null) {
					...
					if (__sequencer != null) {
						...
						__sequencerGameObject = Selection.activeGameObject;
...
//				}
		...
		private void Update ()
		{
			...
			if (__sequencerGameObject != null) {
				__sequencer = __sequencerGameObject.GetComponent<Sequencer> ();
				__sequence = __sequencer.sequence;
			}
...
//						return;
			...
			if (!__sequence.isPlaying) {//PLAY
				...
				__sequencer.SequenceNodeStart -= onSequenceNodeStart;
				__sequencer.SequenceNodeStart += onSequenceNodeStart;
//
				__sequencer.SequenceNodeStop -= onSequenceNodeStop;
				__sequencer.SequenceNodeStop += onSequenceNodeStop;
...
//								}
		...
		private void DoSettingsGUI (float width)
		{
			...
			if (GUILayout.Button (__sequence != null ? __sequence.name : "[None Selected]", EditorStyles.toolbarDropDown, GUILayout.Height (20), GUILayout.Width (width * 0.3f))) {
				...
				List<Sequencer> sequencers = GameObjectUtilityEx.FindAllContainComponentOfType<Sequencer> ();
...
//										}
		...
		private void sequenceNodeClickEventHandler (SequenceNode node, Rect clickRect, Rect rect, int channelOrd)
		{
			...
			case EventType.MouseDown:
				...
				float nodeRectWidth = clickRect.width;
				bool canResizeStart = nodeRectWidth > 5 && rect.x <= clickRect.x - 5;
				bool canResizeEnd = nodeRectWidth > 5 && rect.xMax >= clickRect.xMax + 5;
				...
				if (canResizeStart && clickRect.Contains (Event.current.mousePosition)) {
					...
					ev.Use ();
								...
								//move rect at the node rect end  width=10x;
				clickRect.x += nodeRectWidth;
				...
				if (clickRect.Contains (Event.current.mousePosition)) {
					...
					if (ev.button == 0) {
						...
						if (nodeRectWidth > 30f && node.channel.type == SequenceChannel.SequenceChannelType.Animation && !String.IsNullOrEmpty (__nodeSelectedSourceEditorCurveBinding.propertyName)) {
							...
							curveEditorRect.height = Screen.height - curveEditorRect.yMin;
							curveEditorRect.y = curveEditorRect.y + window.position.yMin;//make it global
								
							//curveEditorRect.width=rect.width * 0.8f;
...
//										selectedNode.channel = Mathf.Clamp (selectedNode.channel, 0, int.MaxValue);
		...
		private static void RemoveNode (SequenceNode node, SequenceChannel sequenceChannel)
		{
			...
			DestroyImmediate (node.clipBinding, true);
			...
			if (sequenceChannel.nodes.Count == 0) {
				...
				DestroyImmediate (sequenceChannel, true);
				
			} 

			//reimport/refresh sequence cos of deletion
			AssetDatabase.ImportAsset (AssetDatabase.GetAssetPath (__sequence));
		...
		/// </summary>
		/// <param name="data">Data.</param>
		private void OnGameObjectMenuSelectionChanged (object data)
		{
			__sequencer = data as Sequencer;
			Selection.activeGameObject = __sequencer.gameObject;
		...
		private static void AddDropToNewOrExistingChannel (UnityEngine.Object droppedObject, Type type, SequenceChannel sequenceChannel, Vector2 pos)
		{
			...
			if (sequenceChannel == null) {
				...
				AssetDatabase.AddObjectToAsset (sequenceChannel, __sequence);
			} else
		...
		private static SequenceNode CreateNewSequenceNode (Vector2 pos, UnityEngine.Object source, SequenceChannel sequenceChannel, int layerIndex=0)
		{
			...
			AssetDatabase.AddObjectToAsset (node, sequenceChannel);
		...
		private void CreateNewSequence ()
		{
			...
			__sequencerGameObject = new GameObject ("Sequence " + GetNextAvailableOrdNumber ().ToString ());
			__sequencer = __sequencerGameObject.AddComponent<Sequencer> ();
						
						
					
			Selection.activeGameObject = __sequencerGameObject;
		}

		private static int GetNextAvailableOrdNumber ()
		{
			List<Sequencer> sequences = GameObjectUtilityEx.FindAllContainComponentOfType<Sequencer> ();
			int count = 0;
...
//			Undo.postprocessModifications -= PostprocessAnimationRecordingModifications;
		...
		private static void selectionChangeEventHandler ()
		{
			...
			if (activeGameObject != null && activeGameObject != __sequencerGameObject) {

								
								

				__sequencer = activeGameObject.GetComponent<Sequencer> ();
				if (__sequencer != null) {
					__sequencerGameObject = __sequencer.gameObject;
					__sequence = __sequencer.sequence;
